...
async function main() {
  console.log("🌱 Seeding database with analytics mock data...");
  ...
  // Create or get Admin
  let admin = await prisma.coach.findUnique({
    where: { email: "admin@feestrack.com" },
  });

  if (!admin) {
    const adminPassword = await bcrypt.hash("admin123", 10);
    admin = await prisma.coach.create({
      data: {
        fullName: "Admin User",
  ...
  // Create Centres - check if they exist first, then create or update
  let centre3lok = await prisma.center.findFirst({
    where: { name: { contains: "3lok" } },
  });
  if (!centre3lok) {
    centre3lok = await (prisma as any).center.create({
      data: {
  ...
  let centreDepot18 = await prisma.center.findFirst({
    where: { name: { contains: "Depot18" } },
  });
  if (!centreDepot18) {
    centreDepot18 = await (prisma as any).center.create({
      data: {
        name: "Depot18 - Sports",
        shortName: "DEPOT18",
        city: "Bengaluru",
  ...
  let centreBlitzz = await prisma.center.findFirst({
    where: { name: { contains: "Blitzz" } },
  });
  if (!centreBlitzz) {
    centreBlitzz = await (prisma as any).center.create({
      data: {
  ...
  let centreTronic = await prisma.center.findFirst({
    where: { name: { contains: "Tronic" } },
  });
  if (!centreTronic) {
    centreTronic = await (prisma as any).center.create({
      data: {
  ...
  // Ensure all centres exist (non-null assertion for TypeScript)
  if (!centre3lok || !centreDepot18 || !centreBlitzz || !centreTronic) {
    throw new Error("Failed to create or find all required centres");
  }
  ...
  const coach2 = await prisma.coach.upsert({
    where: { email: "dhruv.katyal@fcrb.com" },
    update: {},
    create: {
  ...
  console.log("✅ Created/updated 2 coaches");

  // Create Students
  const studentPassword = await bcrypt.hash("student123", 10);

  const student1 = await prisma.student.upsert({
    where: { email: "arjun.rao@student.fcrb.com" },
    update: {},
    create: {
      fullName: "Arjun Rao",
  ...
  const student2 = await prisma.student.upsert({
    where: { email: "karan.mehta@student.fcrb.com" },
    update: {},
    create: {
  ...
  const student3 = await prisma.student.upsert({
    where: { email: "rahul.sharma@student.fcrb.com" },
    update: {},
    create: {
  ...
  const student4 = await prisma.student.upsert({
    where: { email: "ishaan.verma@student.fcrb.com" },
    update: {},
    create: {
  ...
  const student5 = await prisma.student.upsert({
    where: { email: "rohit.nair@student.fcrb.com" },
    update: {},
    create: {
      fullName: "Rohit Nair",
  ...
  console.log("✅ Created/updated 5 students");
  ...
  // Arjun (student1) - HIGH attendance
  await prisma.attendance.createMany({
    data: [
      ...
      // Rohit (student5) - MODERATE attendance
      {
        sessionId: session1.id,
      ...
      // Karan (student2) - MOD/LOW attendance
      {
        sessionId: session3.id,
        studentId: student2.id,
        status: "PRESENT",
      },
      // Rahul (student3) - LOW attendance
      {
        sessionId: session4.id,
        studentId: student3.id,
        status: "ABSENT",
      },
      // Ishaan (student4) - some presence
      {
  ...
  await (prisma as any).fixturePlayer.createMany({
    data: [
  ...
  await (prisma as any).wellnessCheck.createMany({
    data: [
      // Arjun – high exertion but high energy
      {
        studentId: student1.id,
        sessionId: session1.id,
  ...
  await (prisma as any).monthlyFeedback.createMany({
    data: [
      {
        studentId: student1.id,
        coachId: coach1.id,
  ...
  await (prisma as any).progressRoadmap.upsert({
