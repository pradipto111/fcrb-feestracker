 ...
 * 
 * Displays player metrics, readiness index, positional suitability, and timeline charts
 * for students to view their own analytics.
 */
...
interface YourAnalyticsProps {
  refreshKey?: number; // Optional key to force refresh
}

const YourAnalytics: React.FC<YourAnalyticsProps> = ({ refreshKey = 0 }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const [snapshotData, positionalData, definitionsData] = await Promise.all([
          api.getMyLatestMetricSnapshot().catch((err) => {
            console.error("[YourAnalytics] Error fetching snapshot:", err);
            return { snapshot: null };
          }),
          api.getMyPositionalSuitability().catch((err) => {
            console.error("[YourAnalytics] Error fetching positional:", err);
            return { positional: [] };
          }),
          api.getMetricDefinitions().catch((err) => {
            console.error("[YourAnalytics] Error fetching definitions:", err);
            return { definitions: [] };
          }),
        ]);

        console.log("[YourAnalytics] Snapshot data:", snapshotData);
        console.log("[YourAnalytics] Positional data:", positionalData);

        // Handle snapshot data (null is valid - means no snapshot yet)
        if (snapshotData?.snapshot) {
          console.log("[YourAnalytics] Setting snapshot with", snapshotData.snapshot.values?.length || 0, "metrics");
          setSnapshot(snapshotData.snapshot);
        } else {
          console.log("[YourAnalytics] No snapshot found");
          setSnapshot(null);
        }

        // Handle positional data (empty array is valid)
    ...
    return (
      <Card variant="default" padding="lg" style={{ marginBottom: spacing.lg }}>
        <div style={{ textAlign: "center", padding: spacing.xl }}>
          ...
          <p style={{ marginTop: spacing.lg, color: colors.text.muted }}>Loading your analytics...</p>
        </div>
      </Card>
    );
  }
    ...
    return (
      <Card variant="default" padding="lg" style={{ marginBottom: spacing.lg, background: colors.danger.soft }}>
        ...
        <p style={{ color: colors.text.muted, marginTop: spacing.sm, fontSize: typography.fontSize.sm }}>
          No analytics data available yet. Your coach will add your first assessment soon.
        </p>
      </Card>
    );
  }
      ...
      <Card variant="default" padding="lg" style={{ marginBottom: spacing.lg }}>
        <div style={{ textAlign: "center", padding: spacing.xl }}>
          ...
          <p style={{ ...typography.body, color: colors.text.muted }}>
            Your coach hasn't created your first assessment yet. Check back soon!
          </p>
        </div>
      </Card>
    );
